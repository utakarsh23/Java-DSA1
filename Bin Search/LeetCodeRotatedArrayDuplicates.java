...
public class LeetCodeRotatedArrayDuplicates {
    ...
    static int pivotInRotatedArrayWithDuplicates(int[] nums) {
    /*to check Pivot, some cases can be defined
      i) when middle element > middle element+1 (nums[mid] > nums[mid + 1]), that is the pivot is found, and it is the middle element
      ii) when middle element < middle element-1 (nums[mid] < nums[mid - 1]). i.e : Pivot is found, and it is the element before middle element(mid-1_
      iii)
        */
        ...
        while (start < end) {
            ...
            //3rd case here
            //if element at star, end and mid are equal then skip the duplicates
            if(nums[mid] == nums[start] && nums[mid] == nums[end]) {
                //skipping the duplicates
                //NOTE: what if these elements at start and end were to be pivot?
                //check if they are pivot or not
                if(nums[start] > nums[start+1]) {
                    return start;
                }
                start++;
                //check the end is pivot or not
